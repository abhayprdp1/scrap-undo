...
export default function HomePage() {
  ...
  return (
    <div className="flex flex-col min-h-screen gradient-mesh">
      ...
      {/* Top Ticker: Live Kerala Scrap Rates */}
      <div className="w-full bg-scrap-card/90 border-b border-scrap-border py-2 px-4 overflow-x-auto no-scrollbar flex items-center justify-between text-xs">
        <div className="flex items-center gap-2 pr-4 border-r border-scrap-border whitespace-nowrap">
          <span className="w-2 h-2 rounded-full bg-scrap-primary animate-ping" />
          <span className="font-semibold text-scrap-primary uppercase tracking-wider text-[11px]">Kerala Daily Scrap Rates</span>
          <span className="text-scrap-muted font-normal">({selectedCity})</span>
        </div>
        <div className="flex items-center gap-6 overflow-x-auto scrollbar-none whitespace-nowrap py-0.5 px-2">
          {liveRatesKerala.map((r, i) => (
            <div key={i} className="flex items-center gap-1.5 text-scrap-light">
              <span className="text-scrap-muted">{r.item}:</span>
              <span className="font-semibold text-white">{r.rate}/{r.unit}</span>
              <span className={`text-[10px] ${r.trend.startsWith('+') ? 'text-scrap-primary' : 'text-scrap-gold'}`}>
                {r.trend}
              </span>
            </div>
          ))}
        </div>
        <Link href="/rates" className="hidden sm:flex items-center gap-1 text-scrap-primary hover:underline whitespace-nowrap pl-4 text-[11px] font-medium">
